...
namespace kssm.be.applications.Signing.Interfaces
{
    /// <summary>
    ...
    {
        /// <summary>
        /// Returns a paged certificate list; TotalItems counts every match so the caller can page without a
        /// second call. Keeps certificates that cannot sign, with CanSign and Reason, so the user learns why
        /// one is unavailable instead of silently missing it; OnlySignable = true drops them.
        /// Paging is optional: a caller that sends no pageSize gets the whole list, which is why PageSize 0
        /// means "all" here rather than an empty page.
        /// </summary>
        BaseResponsePagingDto<SignCertDto> GetCertificates(SignCertQueryDto query);
