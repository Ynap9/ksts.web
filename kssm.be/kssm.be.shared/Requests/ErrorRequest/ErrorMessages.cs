namespace kssm.be.shared.Requests.ErrorRequest
{
    public static class ErrorMessages
    {
        private static readonly Dictionary<int, string> _messages = new()
        {
            ...
            { ErrorCodes.KiemTraPhienNotFound, "Không tìm thấy phiên kiểm tra" },
            { ErrorCodes.KiemTraPhienDangChay, "Phiên kiểm tra đang chạy" },
            { ErrorCodes.KiemTraPhienDaDon, "Phiên đã được dọn, cần tải lại thư mục" },
            { ErrorCodes.KiemTraPhienRong, "Phiên chưa nhận được file nào" },
            { ErrorCodes.KiemTraExcelNotFound, "Không tìm thấy file Excel metadata trong thư mục" },
            { ErrorCodes.KiemTraExcelUnreadable, "Không đọc được file Excel metadata" },
            { ErrorCodes.KiemTraExcelLegacyFormat, "Chỉ hỗ trợ file Excel định dạng .xlsx" },
            { ErrorCodes.KiemTraSheetMissing, "File Excel thiếu sheet cần thiết" },
            { ErrorCodes.KiemTraThieuTruongBatBuoc, "File Excel thiếu trường bắt buộc của chuẩn đóng gói" },
            { ErrorCodes.KiemTraMaHoSoKhongKhop, "Mã hồ sơ không khớp với tên thư mục" },
            { ErrorCodes.KiemTraFileMoCoi, "File không có dòng metadata tương ứng" },
            { ErrorCodes.KiemTraTrungMa, "Có mã bị trùng trong thư mục hoặc trong file Excel" },
            { ErrorCodes.KiemTraVuotTranMoiDot, "Vượt quá số file cho phép trong một đợt tải lên" },
            { ErrorCodes.KiemTraDinhDangKhongHoTro, "Định dạng file không được hỗ trợ" },
            { ErrorCodes.KiemTraChuanChuaCauHinh, "Chuẩn đóng gói và loại đối tượng này chưa có cấu hình trường" },
            { ErrorCodes.KiemTraThieuCapGoi, "Phải chọn một loại đối tượng ở cấp gói" },
            { ErrorCodes.KiemTraNhieuCapGoi, "Chỉ được chọn một loại đối tượng ở cấp gói" },
            { ErrorCodes.KiemTraThieuCapTaiLieu, "Phải chọn ít nhất một loại đối tượng ở cấp tài liệu" },
            { ErrorCodes.DongGoiPhienChuaKiem, "Phiên chưa kiểm tra xong nên chưa đóng gói được" },
            { ErrorCodes.DongGoiKhongCoHoSoDat, "Không có hồ sơ nào đạt để đóng gói" },
