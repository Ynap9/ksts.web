namespace kssm.be.shared.Constants.Signing
{
    ...
    /// phần nhận diện provider của token nằm ở <see cref="CertificateConstants"/>.
    /// </summary>
    public static class SigningConstants
    {
        ...
        // Bốn số của /ByteRange: hai đoạn nội dung được chữ ký phủ, chừa lỗ /Contents ở giữa. Đọc lại từ
        // chính file đã ghi ra khi kiểm chứng, không tin số đã tính lúc dựng.
        public const string ByteRangePattern = @"/ByteRange\s*\[\s*(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s*\]";

        // Khối CMS nằm trong /Contents dạng hex, đuôi đệm '0' cho vừa chỗ đã chừa sẵn.
        public const string ContentsPattern = @"/Contents\s*<([0-9A-Fa-f]+)>";
