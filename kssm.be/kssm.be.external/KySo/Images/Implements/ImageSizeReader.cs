using kssm.be.external.KySo.Images.Dtos;
...
namespace kssm.be.external.KySo.Images.Implements
{
    /// <summary>
    ...
    public class ImageSizeReader : IImageSizeReader
    {
        ...
        public ImageSizeDto Read(byte[] bytes)
        {
            ...
            if (size == null)
            {
                throw new UserFriendlyException(ErrorCodes.ImageSizeUnreadable,
                    "Không đọc được kích thước ảnh. Chỉ hỗ trợ PNG và JPEG.");
            }

            if (!size.HasDpiMetadata)
            {
                size.DpiX = ImagePlacementConstants.DefaultImageDpi;
                size.DpiY = ImagePlacementConstants.DefaultImageDpi;
            }

            size.WidthPoints = size.PixelWidth / size.DpiX * ImagePlacementConstants.PointsPerInch;
            size.HeightPoints = size.PixelHeight / size.DpiY * ImagePlacementConstants.PointsPerInch;
            return size;
        }
